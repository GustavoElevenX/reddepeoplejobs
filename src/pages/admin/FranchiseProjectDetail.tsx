...
export function FranchiseProjectDetail() {
  ...
  return <div className="grid gap-6">
    ...
    <Card className="p-3"><div className="flex gap-2 overflow-x-auto">{tabs.map((tab) =>
      <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`min-w-max rounded-lg px-3 py-2 text-sm font-bold ${activeTab === tab ? 'bg-redde-500 text-white' : 'bg-surface-50 text-ink-700'}`}>{tab}</button>)}</div></Card>
    <Card className="p-6">
      <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-lg bg-redde-50 text-redde-600"><MessageSquareText size={20}/></span>
        <div><h2 className="text-xl font-black text-ink-900">{activeTab}</h2><p className="text-sm text-ink-500">{simpleText[activeTab]}</p></div></div>
      ...
      {(activeTab === 'Candidatos' || activeTab === 'Ranking') ? <div className="mt-5 grid gap-4">{model.applications.map((application) => {
        ...
        return <div key={application.id} className="rounded-lg border border-surface-200 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black text-ink-900">{application.candidate_name}</p>
            <p className="text-sm text-ink-500">{application.status} · {ranking.score}% · {ranking.source === 'ai' ? 'Ranking por IA persistido' : 'Estimativa preliminar'}</p></div>
      ...
      {activeTab === 'Triagem' ? <div className="mt-5 grid gap-3">{model.applications.map((application) => {
        ...
        return <div key={application.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4"><div><p className="font-bold">{application.candidate_name}</p><p className="text-sm text-ink-500">{row?.status ?? 'não iniciada'} · análise {application.resume_analysis_status}</p></div><Button size="sm" onClick={() => void openScreening(application.id)}>Abrir triagem</Button></div>;
      })}</div> : null}
      ...
      {activeTab === 'Entrevistas internas' ? <div className="mt-5 grid gap-3">{model.screenings.filter((item) => item.status === 'completed').map((row) => {
        ...
        return <div key={row.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4"><div><p className="font-bold">{application?.candidate_name}</p><p className="text-sm text-ink-500">{current?.status ?? 'não iniciada'} · {current?.recommendation ?? 'sem recomendação'}</p></div><Button size="sm" onClick={() => void openInterview(row.application_id)}>Abrir entrevista</Button></div>;
      })}</div> : null}
      ...
      {activeTab === 'Finalistas' ? <div className="mt-5 grid gap-4">{model.interviews.filter((item) => item.status === 'completed' && item.recommendation !== 'no').map((row) => {
        ...
        return <div key={row.id} className="rounded-lg border p-4"><p className="font-black">{application?.candidate_name}</p><p className="mt-1 text-sm text-ink-500">Parecer: {finalist?.ai_report_status ?? 'não gerado'} · Seleção: {finalist?.status ?? 'pendente'}</p>
      ...
      {activeTab === 'Aprovação do cliente' ? <div className="mt-5 grid gap-4"><div className="rounded-lg border p-4"><p className="font-bold">{activeFinalists.length} finalista(s) selecionado(s)</p><p className="text-sm text-ink-500">Todos precisam de parecer aprovado. Para liberar menos de três, registre uma justificativa.</p>
        ...
        {model.finalists.map((item) => <div key={item.id} className="rounded border p-3 text-sm">{model.applications.find((app) => app.id === item.application_id)?.candidate_name} · {item.status} · parecer {item.ai_report_status}</div>)}</div> : null}
      ...
      {activeTab === 'Contrato / OS / Financeiro' ? <div className="mt-5 grid gap-4"><div className="grid gap-3 sm:grid-cols-3"><Card className="p-3"><p className="text-xs text-ink-500">Recebido</p><p className="font-black">{money(received)}</p></Card><Card className="p-3"><p className="text-xs text-ink-500">A receber</p><p className="font-black">{money(open)}</p></Card><Card className="p-3"><p className="text-xs text-ink-500">Saldo futuro</p><p className="font-black">{money(future)}</p></Card></div>
        <p className="text-sm text-ink-600">Contrato: {model.contract?.status ?? '-'} · assinatura {model.contract?.provider ? model.contract.signature_status : 'externa/manual'} · OS {money(model.serviceOrder?.amount)}</p>
        {model.receivables.map((receivable) => <div key={receivable.id} className="rounded-lg border p-4"><p className="font-bold">{receivable.description} · {money(receivable.total_amount)}</p><div className="mt-3 grid gap-3">{model.installments.filter((item) => item.receivable_id === receivable.id).map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded bg-surface-50 p-3"><div><p className="font-bold">{item.description} · {money(item.amount)}</p><p className="text-xs text-ink-500">{item.status} · vencimento {item.due_date ?? '-'} · gatilho {item.release_trigger}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" disabled={item.status === 'locked'} onClick={() => void action(() => updateReceivableInstallment(item.id, { status: 'received' }), 'Parcela marcada como recebida.')}>Recebida</Button><Button size="sm" variant="secondary" disabled={item.status === 'locked'} onClick={() => void action(() => updateReceivableInstallment(item.id, { status: 'overdue' }), 'Parcela marcada como vencida.')}>Vencida</Button><label className="cursor-pointer rounded-lg border bg-white px-3 py-2 text-sm font-bold">Comprovante<input className="hidden" type="file" onChange={(event) => void uploadReceipt(event, item.id)}/></label></div></div>)}</div></div>)}</div> : null}

      {activeTab === 'Aprovação final' ? <div className="mt-5 grid gap-3"><p className="font-bold">Status conjunto: {model.project.client_decision_status}</p>{model.decisions.map((item) => <div key={item.id} className="rounded border p-3"><p className="font-bold">{model.applications.find((app) => app.id === item.application_id)?.candidate_name} · {item.decision}</p><p className="text-sm text-ink-500">{item.is_final ? `Finalizada em ${item.finalized_at ? new Date(item.finalized_at).toLocaleString('pt-BR') : '-'}` : 'Decisão salva como rascunho'} · início {item.start_date ?? '-'}</p></div>)}</div> : null}
      ...
      {activeTab === 'Pós-venda' ? <div className="mt-5 grid gap-3">{model.postSaleTasks.map((task) => <div key={task.id} className="flex items-center justify-between gap-3 rounded border p-3"><div><p className="font-bold">{task.title}</p><p className="text-sm text-ink-500">{task.status} · {task.due_date}</p></div><Button size="sm" onClick={() => setPostSale(task)}>Registrar contato</Button></div>)}</div> : null}
      {activeTab === 'Agenda' ? <div className="mt-5 grid gap-3">{model.schedules.map((item) => <div key={item.id} className="rounded border p-3"><p className="font-bold">{item.date} às {item.time}</p><p className="text-sm text-ink-500">{item.format} · {item.location_or_link} · {item.candidate_confirmed_at ? 'confirmada' : 'aguardando confirmação'}</p></div>)}</div> : null}
