...
export function ProcessSelectionTab(props: Props) {
  ...
  return <section className="grid min-w-0 gap-3">
    <div className="flex justify-end gap-2"><Button size="sm" variant={view === 'kanban' ? 'primary' : 'secondary'} onClick={() => setView('kanban')}><Columns3 size={15} />Kanban</Button><Button size="sm" variant={view === 'list' ? 'primary' : 'secondary'} onClick={() => setView('list')}><List size={15} />Lista</Button></div>
