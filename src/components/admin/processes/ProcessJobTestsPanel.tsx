...
export function ProcessJobTestsPanel({ jobId, franchiseId, canManage }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : 'Não foi possível carregar testes e prazos.');
  ...
  if (loading) return <div className="rounded-xl border border-surface-200 bg-white p-5 text-sm font-semibold text-ink-500">Carregando testes e prazos...</div>;
  return <div className="grid gap-3">
    ...
    <section className="rounded-xl border border-surface-200 bg-white shadow-card"><div className="px-4 py-3"><h3 className="font-black text-ink-900">Prazo por etapa</h3><p className="text-xs text-ink-500">O prazo é recalculado automaticamente a cada movimentação.</p></div><div className="divide-y divide-surface-200 border-t border-surface-200">{activeStages.map((stage) => { const setting = slaSettings.find((item) => item.stage === stage); return <div key={stage} className="grid items-end gap-3 px-4 py-3 sm:grid-cols-[1fr_120px_120px_auto]"><p className="pb-2 text-sm font-bold text-ink-900">{applicationStageLabels[stage]}</p><Input aria-label={`Prazo de ${applicationStageLabels[stage]}`} label="Dias" type="number" min={0} value={setting?.sla_days ?? ''} onChange={(event) => { const value = Number(event.target.value); setSlaSettings((current) => [...current.filter((item) => item.stage !== stage), { id: setting?.id ?? '', franchise_id: franchiseId, job_id: jobId, stage, sla_days: value, warning_days: setting?.warning_days ?? 1, created_at: setting?.created_at ?? '', updated_at: setting?.updated_at ?? '' }]); }} /><Input aria-label={`Alerta de ${applicationStageLabels[stage]}`} label="Alerta" type="number" min={0} value={setting?.warning_days ?? 1} onChange={(event) => { const value = Number(event.target.value); setSlaSettings((current) => [...current.filter((item) => item.stage !== stage), { id: setting?.id ?? '', franchise_id: franchiseId, job_id: jobId, stage, sla_days: setting?.sla_days ?? 0, warning_days: value, created_at: setting?.created_at ?? '', updated_at: setting?.updated_at ?? '' }]); }} />{canManage ? <Button size="sm" variant="secondary" disabled={saving || !franchiseId || setting?.sla_days === undefined} onClick={async () => { if (!setting) return; setSaving(true); setError(''); try { await upsertJobStageSlaSettings([{ franchise_id: franchiseId, job_id: jobId, stage, sla_days: setting.sla_days, warning_days: setting.warning_days }]); await load(); } catch (saveError) { setError(saveError instanceof Error ? saveError.message : 'Não foi possível salvar o prazo.'); } finally { setSaving(false); } }}>Salvar</Button> : null}</div>; })}</div></section>
  </div>;
}
